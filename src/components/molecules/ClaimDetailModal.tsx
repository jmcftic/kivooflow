...
const ClaimDetailModal: React.FC<ClaimDetailModalProps> = ({
  ...
  const formatPeriodDate = (dateStr: string | undefined): string => {
    ...
    try {
      ...
      const shouldShowUTC = /^\d{4}-\d{2}-\d{2}$/.test(dateStr) ||
        dateStr.includes('T00:00:00') ||
        dateStr.includes('T23:59:59');
                        ...
                        <span className="text-white text-sm">
                          {(() => {
                            try {
                              ...
                              // Determinar si debemos usar UTC para evitar saltos de día por zona horaria.
                              const shouldShowUTC = /^\d{4}-\d{2}-\d{2}$/.test(fecha) ||
                                fecha.includes('T00:00:00') ||
                                fecha.includes('T23:59:59');
